...
if( $regpassword == $regpasswordagain ) {
	...
	$mailcontrol = DB::getRow("SELECT * FROM admins WHERE adminemail=?",array(
            $regemail
    ));
	...
	if ($mailcontrol>0) {
		Header("Location:http://localhost/RentACarProject/register.php?case=notmail");
		exit();
	}
	...
	if($adduser>0){
	    ...
	    $MesajHazirla = "Merhaba Sayın " . $regnamesurname . " Sitemize yapmış olduğunuz üyelik kaydınıızı tamamlamak için <a href='localhost/RentACarProject/Class/activation.php?activation=" . $Activation . "&email=" . $regemail . "'>Buraya Tıklayınız</a>
		...
		Header("Location:http://localhost/RentACarProject/register.php?case=succes");    
	}else{
	    Header("Location:http://localhost/RentACarProject/register.php?case=danger");    
	}

}else{
    Header("Location:http://localhost/RentACarProject/register.php?case=password-again");
}
